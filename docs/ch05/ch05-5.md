        ...
        if n == 0:
            ...
            for i in range(0,n):
                ...
                if d > 0:
                    P1.append(P[i])
                    if abs(d) > pmax1: 
                        #这里只取了大于好，因为当最远距离有多个相同点时，选取角pmaxp1pn最大的点，
                        #即越接近p1的点，遍历从0开始默认保留距离p1接近的点
                        pmax1 = abs(d)
                        pmind1 = i
                else:
                    P2.append(P[i])
            ...
            if pmax2 == 0:
                P2 = []
            return [P[pmind1],P[pmind2]],P1,P2
                
            
    def QuickHull(P,p0,pn):
        n = len(P)
        ...
        if len(P1) == 0:
            ...
            print('P1',P1)
            s11 = QuickHull(P1,p0,Pmax[0])
            s12 = QuickHull(P1,Pmax[0],pn)  
        if len(P2) == 0:
            s21, s22 = [],[]
        elif len(P2) == 1:
            s21 = P2
            ...
            print('P2',P2)
            s21 = QuickHull(P2,p0,Pmax[1])
            s22 = QuickHull(P2,Pmax[1],pn)
        return s11 + s12 + s21 + s22
     
    # test
    # 生成点集
    t = np.random.randint(0, 5, size = (2, 7))
    P = t.T
